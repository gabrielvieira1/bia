...
create_task_definition() {
    ...
    if [ $register_status -ne 0 ]; then
        ...
        log_error "Verifique se você tem permissões para registrar task definitions"
        exit 1
    fi
...
update_service() {
    ...
    local update_result=$(aws ecs update-service \
        ...
        --cluster $cluster \
        --service $service \
        --task-definition $task_family:$revision 2>&1)
    ...
    if [ $wait_status -ne 0 ]; then
        ...
        log_info "O deploy pode ter sido bem-sucedido, verifique o console AWS"
    else
